...
func server_despawn():
	if multiplayer.is_server():
		...
		#FIXME: Unless this delat is added, the Main trees PickupSpawner may delete a Pickup on the server side, before the player script has a chance
		...
		get_tree().create_timer(1).timeout.connect(callable_pickup_destroy, CONNECT_ONE_SHOT)
